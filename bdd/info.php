<?php
    ...
    $voiture = $mysqli->query("SELECT COUNT(sre_id) FROM t_soiree_sre WHERE sre_voiture ='1' AND sre_confirmation ='1'");
    $place = $mysqli->query("SELECT SUM(sre_place) FROM t_soiree_sre WHERE sre_confirmation ='1'");
    $matelas = $mysqli->query("SELECT SUM(sre_matelas) FROM t_soiree_sre WHERE sre_confirmation ='1'");
